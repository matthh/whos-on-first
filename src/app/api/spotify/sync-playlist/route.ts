...
import {
  SPOTIFY_API_BASE,
  DEFAULT_WALK_UP_SONGS,
  findCleanTrack,
  getValidServiceAccessToken,
  getSpotifyServiceUserId,
} from "@/lib/spotify";
...
export async function POST(request: NextRequest): Promise<NextResponse<SyncResult>> {
  ...
  // Use the app-owner's Spotify service account for ALL teams. One Spotify
  // identity hosts every walk-on playlist; coaches don't authorize Spotify.
  const token = await getValidServiceAccessToken();
  if (!token) return NextResponse.json({ ok: false, reason: "spotify_service_not_linked" });

  const spotifyUserId = await getSpotifyServiceUserId();
  if (!spotifyUserId) return NextResponse.json({ ok: false, reason: "spotify_service_not_linked" });
  ...
  // Disambiguate by team id so two teams named "Astros" don't collide on
  // the service account. Coaches still see and share the playlist by URL.
  const playlistName = `${team.name} Walk On Music (#${team.id})`;
